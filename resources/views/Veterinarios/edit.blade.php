...
<head>
    ...
    <title>Modificar datos del veterinario</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Modificar datos del veterinario</h1>
        <form action="{{ route('Veterinario.update', $Veterinario->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div>
                <label for="nombre">Nombre:</label>
                <input type="text" name="nombre" id="nombre" value="{{ $Veterinario->nombre }}" required>
            </div>
            <div>
                <label for="apellido">Apellido:</label>
                <input type="text" name="apellido" id="apellido" value="{{ $Veterinario->apellido }}" required>
            </div>           
